...
class AskService:
    ...
    def ask(db, payload: AskRequest) -> AskResponse:
        ...
        try:
            ...
            retrieved_chunk_ids = [item.chunk_id for item in retrieval_response.results]
            reranked_chunk_ids = [item.chunk_id for item in reranked_results]
            cited_chunk_ids = [item.chunk_id for item in citations]
            latency_ms = int((time.perf_counter() - started_at) * 1000)
            AskRunsService.create_ask_run(
                db=db,
                project_id=payload.project_id,
                query=payload.query,
                answer=answer,
        ...
        except Exception as exc:  # noqa: BLE001 - persist failed ask runs
            latency_ms = int((time.perf_counter() - started_at) * 1000)
            AskRunsService.create_ask_run(
                db=db,
                project_id=payload.project_id,
                query=payload.query,
                answer=f"ASK_FAILED: {str(exc)[:1000]}",
                status="failed",
                latency_ms=latency_ms,
                retrieved_chunk_ids=retrieved_chunk_ids,
                reranked_chunk_ids=reranked_chunk_ids,
